    ...
    submit = () => {
      ...
      if (/\.\d+$/g.test(value)) {
        ...
        if (/^[0]\.\d+/g.test(value)) {coma = 'cero coma '} else {coma = " coma "};
    ...
    function formatNumber(n) {
        ...
        var position = enter.selectionStart;
        ...
        enter.setSelectionRange(position, position);
        if (digitSpace && backspace && position > 1) {enter.setSelectionRange(position, position-1)};
        if ((/^\d\d\s/).test(enter.value) && backspace) {enter.setSelectionRange(position, position)};
        if ((/^\d\s\d\d\d/).test(enter.value) && !backspace) {enter.setSelectionRange(position+1, position+1)};
    ...
    function insertSuperIndex(index) {
      if (index == "backspace") {
          enter.value = enter.value.slice(0, -1);
      } else {
          enter.value = enter.value + index;
      }
      submit();
      enter.focus();
  }
